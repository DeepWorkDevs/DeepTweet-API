...
By default, DeepTweet-API uses 'guest' authentication. If however, access to the full set of resources is required, 'user' authentication can be used, which requires the following additional steps post-installtion:
...
The API_KEY generated by logging in is what allows DeepTweet-API to authenticate as a logged in user while interacting with the Twitter API ('user' authentication). As such it is a very sensitive information and therefore, must be stored securely. The following points must be kept in mind while using the API_KEY for 'user' authentication:
...
DeepTweet-API can be used as a dependency for your NodeJS project. In such a case, it is not required to install DeepTweet-API globally and you may install it locally in the root of your project using the command:
...
-   [Getting the reply timeline of the given user](https://rishikant181.github.io/Rettiwt-API/classes/UserService.html#replies)
